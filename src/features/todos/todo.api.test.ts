...
describe('todo api client', () => {
  ...
  it('updates and deletes todos through the item endpoint', async () => {
    fetchMock
    ...
    await expect(
      updateTodo('todo-one', { title: 'Updated title', completed: true })
    ).resolves.toEqual({
      item: {
        id: 'todo-one',
        title: 'Updated title',
    ...
    await expect(deleteTodo('todo-one')).resolves.toEqual({
      item: {
        id: 'todo-one',
        title: 'Updated title',
        completed: true,
